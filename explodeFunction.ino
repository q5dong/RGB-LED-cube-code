...
/*
 * If from on to off, turn that specific position on 
 * that specific layer to HIGH, for both color.
 */
void controlLedSets(boolean state, int layerIdx, int posIdx, int colorIdx1, int colorIdx2){
  if(!state){ // to turn off, that layer must be HIGH
    digitalWrite(positions[posIdx][colorIdx1], HIGH);
    digitalWrite(positions[posIdx][colorIdx2], HIGH);
  } else { // to turn on in a specific layer
    // Set layer HIGH, and position for two colors LOW
    analogWrite(layers[layerIdx], 200);
    digitalWrite(positions[posIdx][colorIdx1], LOW);
    digitalWrite(positions[posIdx][colorIdx2], LOW);
  }
}
...
void switchColor(int posIdx, int layerIdx, int oldColor, int newColor){
  controlLedSets(0, layerIdx, posIdx, oldColor, oldColor);
  controlLedSets(1, layerIdx, posIdx, newColor, newColor);
}


/*
 * Explosion from inner out.
 */
void explode(){
  turnAllToLow();

  // Turn center 8 leds on color blue
  controlLedSets(1, 1, 0, 2, 2);
  controlLedSets(1, 2, 0, 2, 2);
  delay(500);
  //Switch to red
  switchColor(0, 1, 2, 0);
  switchColor(0, 2, 2, 0);
  delay(500);
  //Turn off and on (blink) for 3 times
  int count = 0;
  while(count != 3){
    controlLedSets(0, 1, 0, 2, 2);
    controlLedSets(0, 2, 0, 2, 2);
    delay(50);
    controlLedSets(1, 1, 0, 0, 0);
    controlLedSets(1, 2, 0, 0, 0);
    delay(300);
  }
  //turn off the center 8
  controlLedSets(0, 1, 0, 0, 0);
  controlLedSets(0, 2, 0, 0, 0);
  //turn on the other two layers
  controlLedSets(1, 0, 3, 0, 0);
  controlLedSets(1, 0, 2, 0, 0);
  controlLedSets(1, 0, 1, 0, 0);
  controlLedSets(1, 3, 3, 0, 0);
  controlLedSets(1, 3, 2, 0, 0);
  controlLedSets(1, 3, 1, 0, 0);
  //blink faster and faster
  int delayTime = 500; // Set max delay time
  while(delayTime != 50){
    controlLedSets(0, 0, 3, 0, 0);
    controlLedSets(0, 0, 2, 0, 0);
    controlLedSets(0, 0, 1, 0, 0);
    controlLedSets(0, 3, 3, 0, 0);
    controlLedSets(0, 3, 2, 0, 0);
    controlLedSets(0, 3, 1, 0, 0);
    delay(50);
    controlLedSets(1, 0, 3, 0, 0);
    controlLedSets(1, 0, 2, 0, 0);
    controlLedSets(1, 0, 1, 0, 0);
