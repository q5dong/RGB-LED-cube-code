...
void turnSingleLayerSetOn(int layerIdx, int posIdx, int colorIdx1, int colorIdx2){
   ...
   */
   ...
   digitalWrite(layers[layerIdx], HIGH); // Set the layer input to HIGH

   // Set all other layers input to low
   for(int lI = 0; lI < 4; lI++{
    if(lI != layerIdx){
      digitalWrite(layers[layerIdx], LOW);
    }
   }
   digitalWrite(positions[posIdx][colorIdx], HIGH);
}
...
void spin(boolean direc){
  ...
  for(lyr = 0; lyr < 4; lyr++){
    for(pos = 0; pos < 4; pos++){
      ...
      turnSingleLayerSetOn(lyr, pos, firstColor, secondColor);
      delay(500); // delay after turning on

      // turning this pattern off from the ground
      digitalWrite(positions[pos][firstColor], LOW);
      digitalWrite(positions[pos][secondColor], LOW);
    }
  }
}
